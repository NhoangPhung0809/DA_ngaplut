...
def render_boundary_map(df_predictions):
    """Render bản đồ ranh giới hành chính và tô màu dạng nhiệt theo xác suất ngập."""
    ...
    if geojson_data and geojson_data.get("features") and geojson_is_in_target_region(geojson_data):
        ...
        # Nếu chưa có GeoJSON hợp lệ thì fallback sang marker vị trí trung tâm.
        for _, row in df_predictions.iterrows():
            ...
            folium.Marker(
                ...
                icon=folium.DivIcon(
                    html=(
                        ...
                        "text-shadow: 0 0 3px black, 0 0 6px black; white-space: nowrap; "
                        "transform: translate(12px, -6px);'>"
                        f"{row['Địa phương']}</div>"
                    )
                ),
            ).add_to(base_map)
...
def main():
    ...
    with map_col:
        ...
        st_folium(flood_map, width=None, height=560, use_container_width=True)
